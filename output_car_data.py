...
def output_car_data(step, project_path):
    position_data = pd.DataFrame(
        columns=['car_num', 'x_position', 'y_position', 'x_acce(m^2/s)', 'y_acce(m^2/s)', 'length(m)', 'speed(m/s)',
                 'LateralSpeed(m/s)', 'accelaration(m^2/s)', 'angel(du)', 'roadID', 'LaneID', 'Lane_index',
                 'lane_position'], dtype=float)
    ...
    for i in all_vehicle_id:
        ...
        position_data.loc[n] = [all_vehicle_id[n], all_vehicle_position[0], all_vehicle_position[1], x_acce, y_acce,
                                get_vehicle_length, get_speed, get_lateral_speed, all_vehicle_accelatatioin, get_angle,
                                get_roadID, get_laneID, get_lane_index, get_lane_position]
        n += 1
    # print(position_data)
    try:
