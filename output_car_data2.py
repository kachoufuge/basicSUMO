...
def output_car_data2(step, project_path):
    position_data = pd.DataFrame(
        columns=['simu_time', 'car_num', 'x_position', 'y_position', 'length(m)', 'speed(m/s)', 'LateralSpeed(m/s)',
                 'accelaration(m^2/s)', 'angel(du)', 'roadID', 'LaneID', 'Lane_index', 'lane_position'], dtype=float)
